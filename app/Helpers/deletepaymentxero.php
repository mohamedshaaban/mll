<?php

use App\User;
use Carbon\Carbon;
use TapPayments\GoSell;
use App\Models\Orders;

if ( ! function_exists( 'deletepaymentxero' ) ) {
    /**
     * Get Total Refunded Amount order
     * @param $id
     *
     * @return  float|integer
     */
    function deletepaymentxero( $data ) {
        generatexerotoken();

        $lineItems = [] ;
        $Reference = '';
        $order = \App\Models\Invoices::find($data);


        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://api.xero.com/api.xro/2.0/Invoices/'.$order->invoice_unique_id,
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
            CURLOPT_FOLLOWLOCATION => true,
            CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
            CURLOPT_CUSTOMREQUEST => 'GEt',
            CURLOPT_HTTPHEADER => array(
                'xero-tenant-id: '.config('app.XERO_TENANT_ID'),
                'Authorization: Bearer '.session('xero_token'),
                'Accept: application/json',
                'Content-Type: application/json',
                'Cookie: _abck=3BE5A6A05BAF5AB8553BA77E30FB8A3F~-1~~-1~-1~-1; ak_bmsc=C70C0DC1D588A1C8CEEAC75CDDD8546B~000000000000000000000000000000~; bm_sz=F3E197CF35509D745E364DF07E089962~YAAQnapkXy8vk2N7AQAAORektA1LcoR92ygBnq7W/AMkk35+XEmGpx9PB7dNw0lhE+p2B92FJRShAGU9mqX7rokkHEWL/jSSb/PTDKuIEVrlkkJ8cNWfFn/DHEEQ2OUivm0ZVAgECgoToph6cO2jB/1+WA9AIWCPpM1qHC+Vll0HeUqxwhbBwjwKI6Nhxg=='
            ),
        ));

        $response = curl_exec($curl);

        curl_close($curl);
        $response = json_decode($response);
        if(isset($response->Invoices[0]->Payments )) {
            foreach ($response->Invoices[0]->Payments as $payment) {
                $dcurl = curl_init();
                curl_setopt_array($dcurl, array(
                    CURLOPT_URL => 'https://api.xero.com/api.xro/2.0/Payments/' . $payment->PaymentID,
                    CURLOPT_RETURNTRANSFER => true,
                    CURLOPT_ENCODING => '',
                    CURLOPT_MAXREDIRS => 10,
                    CURLOPT_TIMEOUT => 0,
                    CURLOPT_FOLLOWLOCATION => true,
                    CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                    CURLOPT_CUSTOMREQUEST => 'POST',
                    CURLOPT_POSTFIELDS => '{ "Payments":[ { "Status":"DELETED" } ] }',
                    CURLOPT_HTTPHEADER => array(
                        'xero-tenant-id: ' . config('app.XERO_TENANT_ID'),
                        'Authorization: Bearer ' . session('xero_token'),
                        'Accept: application/json',
                        'Content-Type: application/json',
                    ),
                ));

                $dresponse = curl_exec($dcurl);
                curl_close($dcurl);
                $dresponse = json_decode($dresponse);

            }
        }
        if(isset($response->Title) && $response->Title =='Unauthorized')
        {
//            generatexerotoken();
//            return   addpaymentxero( $data ,$isOrder , $amount , $code);

        }
         return $response;

    }

























}
