...
if ( ! function_exists( 'addpaymentxero' ) ) {
    ...
    function addpaymentinvoicexero( $data ,$isOrder , $amount , $code,$date) {
        generatexerotoken();
        ...
        curl_setopt_array($curl, array(
            ...
            CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
            CURLOPT_CUSTOMREQUEST => 'POST',
            CURLOPT_POSTFIELDS => '{ "Payments": [ { "Invoice": {  "InvoiceID": "'.$order->xero_id.'" }, "Account": { "Code": "'.$code.'" }, "Date": "'.$date.'","Reference":"'.$Reference.'", "Amount": "'.$amount.'" } ] }',
            CURLOPT_HTTPHEADER => array(
                'xero-tenant-id: '.config('app.XERO_TENANT_ID'),
                'Authorization: Bearer '.session('xero_token'),
                'Accept: application/json',
